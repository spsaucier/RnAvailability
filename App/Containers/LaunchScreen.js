...
// For API
import API from '../Services/GatherApi'
import FJSON from 'format-json'

// Styles
import styles from '../../ignite/DevScreens/Styles/APITestingScreenStyles'
...
export default class APITestingScreen extends React.Component {
  ...
  applyResult (response, title = 'Response') {
    this.refs.container.scrollTo({x: 0, y: 0, animated: true})
    if (response.ok) {
      if (response.data.data.available) {
        // Result of Rooms Availability check
        this.setState({availableRooms: response.data.data.available})
      }
      if (response.data.data[0]) {
        // Result of Packages Availability check
        this.setState({availablePackages: response.data.data})
      }
    } else {
  ...
  tryEndpoint (apiEndpoint) {
    ...
    this.api[endpoint].apply(this, args).then((result) => {
      this.applyResult(result, label || `${endpoint}(${args.join(', ')})`)
    })
  }
  ...
  renderRooms (rooms) {
    return rooms.map((room, index) => {
      return <View style={styles.section} key={index}>
        ...
        <Text style={{marginBottom: 5}}>{room.description}</Text>
  ...
  renderPackages (packages) {
    return packages.map((myPackage, index) => {
      return <View style={styles.section} key={index}>
        ...
        <Text>{myPackage.description}</Text>
        <Text style={{
          color: "#000",
          fontSize: 18,
          fontWeight: "bold",
          marginTop: 5,
        }}>${myPackage.price}</Text>
      </View>
    })
  }
        ...
        { !this.state.availableRooms.length ? 
          <ScrollView style={styles.container} ref='container'>
            <View style={{alignItems: 'center', paddingTop: 60}}>
              <Text style={{fontSize: 28}}>Check Availability</Text>
              <Text>Fleming's Steakhouse Atlanta</Text>
            </View>
            <View style={styles.section}>
              <Text>When would you like to book?</Text>
              <DatePickerIOS
